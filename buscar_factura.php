...
<? if ($ejecuta) { ?>
	...
	<script>
		...
		function bajar_valor_a_cruzar(fact_num, saldo) {
			parent.document.getElementById('factura').value = fact_num;
			parent.document.getElementById('fact_valor').value = saldo;
			parent.CloseAjaxWin();
		}
	</script>
	<!-- Divs contenedores!-->
	...
	<script>
		xajax_reporte_facturas(<?= $idempresa ?>, <?= $sucu ?>, '<?= $clpv ?>', '<?= $factura ?>', '<?= $tran ?>', '<?= $det ?>', '<?= $mone ?>');
	</script>
	<? /********************************************************************/ ?>
	<? /* NO MODIFICAR ESTA SECCION*/ ?>
<? } ?>

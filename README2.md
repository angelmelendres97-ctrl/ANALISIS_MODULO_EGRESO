...
**Cómo leer el modelo**
- Las clases representan tablas Informix/SAE; los atributos corresponden a columnas utilizadas por el proceso de pago.
- Las flechas indican dependencias: `SAESECU` provee secuenciales a `SAEASTO`; la cabecera de asiento (`SAEASTO`) es padre de directorio (`SAEDIR`), detalle diario (`SAEDASI`) y cheques (`SAEDCHC`).
- Las relaciones con `SAECLPV`, `SAEEMAI` y `SAEDIRE` muestran cómo se completan datos del proveedor (RUC, correo, dirección) al momento de guardar un egreso.
...
### Consideraciones adicionales del modelo de datos
- **Integridad referencial implícita**: aunque el código gestiona las claves manualmente, las combinaciones `(asto_cod_empr, asto_cod_sucu, asto_cod_asto)` actúan como claves compuestas en `SAEASTO`, `SAEDIR`, `SAEDASI` y `SAEDCHC`, manteniendo la trazabilidad del comprobante completo.
- **Moneda y cotización**: los campos de montos locales/extranjeros (`dml`, `cml`, `dme`, `cme`) y `tip_camb` en `SAEDASI` permiten recalcular balances a tasa vigente obtenida desde `saetcam`.【F:mayorizacion.inc.php†L19-L27】【F:_Ajax.server.php†L6078-L6100】
- **Control de cheques**: `saectab` conserva el correlativo por cuenta bancaria y se actualiza cada vez que `SAEDCHC` inserta un cheque, evitando duplicidad de numeración en pagos sucesivos.【F:_Ajax.server.php†L6174-L6197】

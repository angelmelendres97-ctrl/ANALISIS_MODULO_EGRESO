...
flowchart LR
    subgraph Navegador
        UI[egreso.php - formulario y JS]
    end
    subgraph Servidor PHP
        XAJ[_Ajax.server.php - controlador Xajax]
        MAY[mayorizacion.inc.php - reglas contables]
        AUX[Scripts de búsqueda / adjuntos]
    end
    subgraph Datos
        DB[(Informix/SAE)]
        Files[(Adjuntos de proveedor)]
    end

    UI -- llamadas asíncronas --> XAJ
    XAJ --> MAY
    XAJ --> AUX
    XAJ -- transacciones --> DB
    MAY -- inserciones/consultas --> DB
    AUX -- lectura/escritura --> DB
    XAJ -- vincula --> Files
```

El diagrama resume cómo la UI dispara llamadas Xajax que llegan a `_Ajax.server.php`, el cual coordina reglas contables en `mayorizacion.inc.php`, actualiza adjuntos y ejecuta transacciones en Informix/SAE. Los scripts de búsqueda se apoyan en la misma conexión (`Dbo`) para poblar autocompletados y grids antes del guardado.
...
### Narrativa integrada del flujo
1. **UI estructurada**: `egreso.php` arma el formulario con secciones de cabecera, grids y botones; valida datos mínimos antes de enviar llamadas Xajax.
2. **Orquestación Xajax**: `_Ajax.server.php` recibe los parámetros, recrea el estado del grid desde sesión, valida diferencias de totales y prepara secuenciales de asiento/comprobante.
3. **Reglas contables reutilizables**: `mayorizacion.inc.php` se encarga de las inserciones en `saeasto` y `saedasi`, asegurando que el asiento contenga moneda, cotización y usuario correctos.
4. **Persistencia y adjuntos**: la misma transacción actualiza documentos pendientes del proveedor y registra cheques cuando es necesario; sólo al final se confirma (commit) y la UI recibe la actualización incremental.
